...
1. **No silent edits to brand-critical data.** Branches must not freely edit product prices, recipes, approved suppliers, promo mechanics, discounts, brand assets, operating procedures, or customer-facing terms. Allow branch-level _requests_ that require head office approval.
...
2. **No automatic legal/financial finality.** The system never auto-declares: final royalty liability, contract breach, tax payable, franchise termination, penalty imposition, or BIR compliance status. It **flags issues and creates review workflows** instead.
   - Bad: _"Branch is non-compliant. Auto-charge penalty."_
   - Good: _"Branch missed required report. Create notice for admin review."_
...
- **Invoice scope**: internal billing only. Every generated invoice has a footer marker: _"Internal billing document — for franchisor/franchisee royalty reconciliation. Not a BIR Official Receipt or Sales Invoice."_ Internal invoice numbering must not mimic BIR series conventions.
